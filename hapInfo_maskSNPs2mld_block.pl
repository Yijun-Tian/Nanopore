...
sub get_methylation_LD_blocks_greedy(){
	...
	for(my $i=0; $i<scalar(@blocks); $i++){
		...
		$hapTable{$probeID}->{'MLD_blocks'}->{$id}->{'rsq_list'} = $words[2] if($debug == 1);
	}
}
...
sub getAllBlockInRegion(){
	...
	if($debug==1){
		my @blocks_with_rsq;
		#make rsq list for each blocks
		foreach my $block_region (@blocks){
			my @rsq_list; 
			my ($block_start, $block_end) = split ":", $block_region;
			for(my $i = $block_start; $i < $block_end; $i++){
				for(my $j = $i+1; $j <= $block_end; $j++){
					my $cur_ld = lookupLD($h_hap_count_table, $i, $j);
					push(@rsq_list, $cur_ld);
				}
			}		
			push(@blocks_with_rsq, $block_start . ":" . $block_end . ":" . join(",", @rsq_list));
		}
		return @blocks_with_rsq;
	}else{
		return @blocks;
	}
}
		...
		for(my $i= $start; $i<= $end-$size+1; $i++){
			...
			for(my $j= $i; $j<$i+$size; $j++){
				for(my $k = $j+1; $k<$i+$size; $k++){
					if((abs($j-$k)<20000) # don't check CpG sites that are too far apart
...
sub lookupLD(){
	...
	if (!$LD_matrix{$locusA}->{$locusB}){	
		...
		$r2=-0.1 if($r2 =~ /NA/ || $total_haps < $min_hap_counts);
		$r2+=0.0001;
...
sub haplotype2LD(){
	...
	return ("NA","NA","NA","NA", $mafA, $mafB) if($mafA <= 0.3 || $mafB <= 0.3);
