...
library(tidyverse)
library(ggplot2)
df <- fread("MA1930.1.CTCF.COLO829.auc_motifctl_scored.bed") %>%
      mutate(motif = as.numeric(V4), meth = as.numeric(V6)) %>%
      mutate(label = case_when(V5 == "vacant" ~ 0, .default = 1)) %>%
      mutate(rule = as.integer(motif == 1 & meth < 23.7125))

#decide best methylation cutoff: 23.7125 for MA1930.1
...
ggplot(df_all, aes(x = FPR, y = TPR, color = Model)) +
  ...
  labs( title = "ROC: MA1930.1", x = "False Positive Rate (1 - Specificity)", y = "True Positive Rate (Sensitivity)", color = "Model") +
  coord_fixed(ratio = 1) +
  theme(
